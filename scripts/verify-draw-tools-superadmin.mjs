...
const OUT = join(ROOT, 'temp_screenshots', 'draw-tools-superadmin');
...
async function main() {
  ...
  const latInput = page.locator('xpath=//label[contains(.,"Latitude")]/following-sibling::input');
  const lngInput = page.locator('xpath=//label[contains(.,"Longitude")]/following-sibling::input');

  // ═══ (a) Incident placement mode ═══
  await page.click('button:has-text("Add Incident")');
  await page.waitForSelector('h2:has-text("Create Incident")', { timeout: 5000 });
  await page.waitForTimeout(400);

  const hintArmed = await page.locator('text=Click on the map to place the incident').isVisible().catch(() => false);
  const cursor = await page.evaluate(() => window.__intelmap24SuperadminMap.getCanvas().style.cursor);
  ...
  await latInput.fill('20.5');
  await lngInput.fill('78.9');
  await page.waitForTimeout(1500);
  const after = await placedMarkerTransform();
  const latVal2 = await latInput.inputValue();
  ...
  const cursorAfter = await page.evaluate(() => window.__intelmap24SuperadminMap.getCanvas().style.cursor);
  ...
  // Cancel the form → placement marker removed
  ...
  const ringInfo = await page.evaluate(() => {
    const src = window.__intelmap24SuperadminMap.getSource('draw-preview');
  ...
  await page.waitForTimeout(1200);
  const zoneFormOpen = await page.locator('text=Create New Zone').isVisible().catch(() => false);

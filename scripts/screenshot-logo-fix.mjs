...
// Sign-in modal (opens from the header Google button)
for (const theme of ['dark']) {
  const page = await newThemedPage(theme);
  await page.goto(`${baseUrl}/`, { waitUntil: 'networkidle' });
  await page.waitForTimeout(1500);
  await page.getByText('Sign in with Google', { exact: true }).first().click();
  await page.waitForTimeout(900);
  await page.screenshot({ path: path.join(outDir, `signin-modal-${theme}.png`), fullPage: false });
  console.log(`Captured signin-modal-${theme}.png`);
  await page.close();
}

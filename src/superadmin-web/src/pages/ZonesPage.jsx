...
export default function ZonesPage() {
  ...
  const handleResolve = async (zone) => {
    ...
    setActionError('');
    try {
      await resolveIncident(zone.id, { resolvedAt: new Date().toISOString() });
      setRefreshKey((k) => k + 1);
    } catch (err) {
                  ...
                  zones.map((zone) => {
                    ...
                    return (
                      <tr
                        key={zone.id}
                        className="tui-row"
                        onClick={() => navigate(`/superadmin/zone/${zone.id}`)}
                      >
                        <td>
                        ...
                        <td className="tui-cell-actions" onClick={(e) => e.stopPropagation()}>
                          <div className="tui-actions">
                            ...
                            <button
                              ...
                              title="View on map"
                              onClick={() => navigate(`/superadmin/map?zone=${zone.id}`)}
                            >
                              <MapPin size={14} />
                            </button>
                            <button

...
export default function MapControls({ dateRange, onDateRangeChange, onResetToToday, verifiedOnly, onVerifiedOnlyChange, status, onStatusChange }) {
  const now = new Date();
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
        {['active', 'resolved', 'all'].map((s) => {
          const isActive = status === s || (s === 'all' && !status);
          const label = s === 'all' ? 'All' : s === 'active' ? 'Active' : 'Resolved';
          return (
            <button
              key={s}
              ...
              style={{
                ...
                letterSpacing: '0.5px',
                borderRadius: 'var(--radius-sm)',
                border: `1px solid ${isActive ? 'var(--navy-500)' : 'var(--border-subtle)'}`,
                background: isActive ? 'rgba(37, 99, 235, 0.12)' : 'var(--bg-input)',
                color: isActive ? 'var(--navy-300)' : 'var(--text-muted)',
                cursor: 'pointer',
                transition: 'all 0.2s ease',
              }}
            >

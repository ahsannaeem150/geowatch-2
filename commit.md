...
## 📅 2026-08-08 — Module: user-web + superadmin-web — Trial CSS leak fix + console accent

fix: user-web zone button light/crimson-hover root cause = stale local ZoneTrial.css/ZoneHeroesTrial.css copies loaded GLOBALLY via 7 eagerly-imported trial pages in App.jsx — all 7 now React.lazy + Suspense (trial CSS code-splits out of main bundle, 283→264 kB). ParticleCanvas.jsx stale crimson fallbacks → turquoise. Audits: admin-web clean (trial CSS was scope-prefixed), superadmin-web clean. feat: superadmin console turquoise accent pass (Option B — navy base kept): sidebar active pill, Map doorway + Create user/Export/New Domain/New Zone Category + modal submit buttons → solid --sa-accent, Users KPI icon turquoise; semantic badge colors untouched. Builds green.

## 📅 2026-08-08 — Module: user-web + backend — Power Search overhaul

feat: (1) New shared BrandHomeLink component (map-topbar cluster: mark + wordmark + hover nav-peek, click → /) extracted from WorkspaceTopBar and now used in PowerSearch topbar too (dead full-lockup replaced). (2) Result cards redesigned to compact 3 rows with zone/incident distinction (zone glyph + zone-category colored vs domain dot + DOMAIN), severity S{n} mini-chip (incidents), verification badge only when ≠ unverified, working selection highlight (was hardcoded null). (3) Zone categories filter added (backend already supported zoneCategoryIds); backend searchIncidents now wraps domain/category-slug conditions polygon-safe so zone results no longer silently vanish under incident filters (benefits all apps). (4) Rail reordered (Type → Date → Incident filters → Zone filters → State → Sources → Options), saved-only sign-in hint when logged out, sort label fix, dead code cleanup. Zones DO carry severity (ZoneEditorSidebar) → severity stays a common filter. Build green; API + headless DOM verified.

style: power search cards now show status only when Active (green dot + label); Resolved renders nothing (same principle as verification badge). Status filter unchanged.

feat: selected-card highlight across user-web map drawer lists (Incidents in Viewport, Active Incidents, Live Activity, Saved — points + zone cards): accent border + subtle tint matching power-search selection, hover border moved off accent so selection stays distinct. Wired from MapPage selectedIncident via WorkspaceDrawer prop-drilling. user-web has NO dedicated Zones drawer page (zones appear only in Saved); admin/superadmin port must also cover their extra pages (Recents etc.).

feat: polygon zones now included in user-web drawer lists — new drawerIncidents memo (points+zones, created_at desc) feeds the viewport drawer (renamed "Incidents in Viewport" → "In Viewport"); Active drawer includes active zones; Layers zone toggles filter the lists too; zone rows use glyph + zone-category color (rows were already zone-aware); empty-state copy updated. Note: 24h+ overdue flag now also applies to long-lived active zones.

## 📅 2026-08-08 — Module: admin-web + superadmin-web — Power Search + drawer port

feat: full user-web power search + drawer overhaul ported to both staff apps: compact 3-row result cards (zone glyph + ZONE CATEGORY vs domain dot + DOMAIN, Active-only status, S{n} chip incidents-only, verification glyph ≠ unverified via shared VERIFICATION_CONFIG — admin's local hex map deleted), real selection wiring (was hardcoded null), dead "G" lockup → BrandLogo mark (click → '/' admin, '/superadmin' console), zoneCategoryIds filter end-to-end + Zone categories section, rail reorder with RailGroupLabel micro-labels, sort default/reset 'newest'. Drawers: selectedIncidentId threaded into all list rows (IncidentCard/ActiveRow/ActivityRow/SavedRow/RecentRow) with accent selected style, hover borders → --border-hover, drawerIncidents memos (Layers-toggle-aware zones, created_at desc); superadmin indigo zone fallback → #6b7280. Staff-only features preserved (Resolve, 24h+, unseen/unread tints, Clear). Builds green; DOM checks 7/7 admin + 5/5 superadmin.
